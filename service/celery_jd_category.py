...
from service.kafka_helper import producer
...
app = Celery(
    'jd_category',
    broker='redis://:@0.0.0.0:6379/2',
    backend='redis://:@0.0.0.0:6379/3'
)


# celery -A service.celery_jd_category.app worker -l info -P gevent -c 60
@app.task(name='jd_category.jd_category')
def get_data(task):
    sku = task.get('sku')
    response = requests.get(f'https://item.jd.com/{sku}.html', headers=headers)
    category = re.findall(r'catName: (.*?),        brand', response.text)[0]
    info = {'sku': sku, 'category': category}
    producer.send('jd_category', value=bytes(json.dumps(info, ensure_ascii=False), encoding='utf8'))
    producer.flush()
